...
watch(
  ...
  (open) => {
    if (open) {
      ...
      form.year = ''
      form.color = ''
      form.colorHex = '#777777'
      form.plate = ''
      form.driverId = null
      form.stashLocation = ''
      form.status = 'In Garage'
    }
  }
)
...
function submit() {
  if (!form.vehicle.trim() || !form.year.trim() || !form.plate.trim()) return
  emit('create', {
...
<template>
  <BaseModal :show="show" title="Add Vehicle" max-width="max-w-3xl" @close="emit('close')">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            :placeholder="locationLocked ? 'Locked for this status' : 'Warehouse 12 — East'"
          />
        </div>
      </div>
