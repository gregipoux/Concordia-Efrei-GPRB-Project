...
| **P4** Security Hardening | Helmet + CORS + rate limiting on `/login`, IDOR audit on intel POST, timing-safe bcrypt compare, online-status sync on login/logout, role-based access control on recruitment **and on every destructive `DELETE` route (missions / vehicles / intel)**, integer coercion on validated FK fields, bcrypt upgrade to 6.x for a clean production audit, automated security test suite (8 cases, `node:test` + `supertest`) | MONTENEGRO LOUREIRO Marco-Antonio | Done |
...
- **Role-based authorization.** A second middleware `requireRole(...roles)` checks `req.agent.role` after the JWT has been verified. It gates `POST /api/agents` (only a `GODFATHER` can create or promote an account) **and every destructive `DELETE` route on the shared resources: `DELETE /api/missions/:id`, `DELETE /api/vehicles/:id` and `DELETE /api/intel/:id`**. An authenticated regular `AGENT` who attempts any of these gets a `403 forbidden`. The principle: regular operatives can read and contribute, but only the Godfather can wipe shared state.
- **Testable application layout.** The Express app is created in `src/app.js` (middlewares, routes, error handler) while `src/index.js` only calls `app.listen`. This split lets `test/security.test.js` import the app directly through `supertest` without binding a port. The suite (`npm test`, on top of `node:test`) covers eight scenarios: missing bearer header, expired token, malformed token, invalid mission enum payload, invalid vehicle FK type, non-Godfather recruitment attempt, **non-Godfather `DELETE` attempts on missions / vehicles / intel (one assertion per resource, all returning `403 forbidden`)** and numeric input sanitization. All eight pass on a clean checkout, and the production `npm audit --omit=dev` reports zero vulnerabilities after upgrading `bcrypt` to `^6.0.0`.
...
| 16 | Role-based access control on recruitment and destructive operations | `requireRole('GODFATHER')` middleware on `POST /api/agents`, `DELETE /api/missions/:id`, `DELETE /api/vehicles/:id` and `DELETE /api/intel/:id` -- only the Godfather can recruit, promote or wipe shared state | Without this, any authenticated agent could recruit (including granting themselves the Godfather role) or destroy missions, vehicles and intel files belonging to the rest of the crew |
...
| 20 | Automated security regressions | `test/security.test.js` (`node:test` + `supertest`) -- 8 scenarios covering auth edge cases, input validation, recruitment authorization and destructive-route authorization | Regressions on the security surface are caught by `npm test` before the next demo |

For the presentation we will live-explain practices **#4 (timing-safe), #7 (rate-limit), #10 (IDOR fix) and #16 (role-based access control on recruitment and destructive `DELETE`s)** because these are the ones a grader is most likely to probe.
...
- **Per-record ownership for `PUT` / `PATCH`.** Recruitment and every destructive `DELETE` are now gated on `GODFATHER`, but updates (`PUT /api/missions/:id`, `PUT /api/vehicles/:id`, `PUT /api/intel/:id`) remain open to any authenticated agent. The natural extension is a per-record ownership model -- the agent who created a record can edit it, the Godfather can edit anything -- which would mirror the destructive-route gate on the modify path. This was deliberately scoped out of Milestone 3: the brief asks for "secure coding practices", which we read as authentication, validation, hardening and protection against the OWASP basics, not a full RBAC matrix on every CRUD verb.
